...
function doRectanglesOverlap(rect1, rect2) {
  ...
  if (firstRight >= rect2.top && firstBottom >= rect2.left) {
    bool = true;
  }
  return bool;
}
...
function isCreditCardNumber(ccn) {
  const ccnStr = String(ccn).split('').reverse();
  const sum = [];
  let result = 0;
  for (let i = 0; i < ccnStr.length; i += 1) {
    let mult = Number(ccnStr[i]);
    if (i % 2 !== 0) {
      mult *= 2;
      if (mult > 9) {
        mult -= 9;
